...
def _make_run_record(session=SESSION, attempt=1):
    rid = make_signal_run_id(session, MODEL, FULL_MODEL_HASH)
...
class TestSignalRunId:
    def test_same_inputs_give_same_id(self):
        id1 = make_signal_run_id(SESSION, MODEL, FULL_MODEL_HASH)
        id2 = make_signal_run_id(SESSION, MODEL, FULL_MODEL_HASH)
        assert id1 == id2

    def test_same_day_retry_is_same_id(self):
        """A retry on the same day with same model must produce the same signal_run_id."""
        id1 = make_signal_run_id(SESSION, MODEL, FULL_MODEL_HASH, "scheduled")
        id2 = make_signal_run_id(SESSION, MODEL, FULL_MODEL_HASH, "scheduled")
        assert id1 == id2

    def test_new_session_gives_new_id(self):
        id1 = make_signal_run_id("2026-08-05", MODEL, FULL_MODEL_HASH)
        id2 = make_signal_run_id("2026-08-06", MODEL, FULL_MODEL_HASH)
        assert id1 != id2

    def test_new_model_version_gives_new_id(self):
        id1 = make_signal_run_id(SESSION, "quant_baseline_v1", FULL_MODEL_HASH)
        id2 = make_signal_run_id(SESSION, "quant_baseline_v2", FULL_MODEL_HASH)
        assert id1 != id2
...
class TestFeatureSnapshotId:
    def test_same_input_gives_same_id(self):
        run_id = make_signal_run_id(SESSION, MODEL, FULL_MODEL_HASH)
...
class TestAppend:
    ...
    def test_different_run_same_ticker_allowed(self, isolated_ledger):
        run1 = make_signal_run_id("2026-08-05", MODEL, FULL_MODEL_HASH)
        run2 = make_signal_run_id("2026-08-06", MODEL, FULL_MODEL_HASH)
...
class TestPartialLineHealing:
    ...
    def test_corrupt_last_line_healed_and_valid_lines_preserved(self, isolated_ledger):
        ...
        ok, bad = check_ledger_integrity("feature_snapshot", YEAR_MONTH)
        assert bad == 0
        assert ok == 2
...
class TestIntegrityCheck:
    ...
    def test_missing_file_returns_zeros(self, isolated_ledger):
        ok, bad = check_ledger_integrity("feature_snapshot", "2099-01")
        assert ok == 0
        assert bad == 0
